...
Run = "Run00005"
angle = 'm1'    # relative to resonance angle
...
Savedata = 1    # Save processed data in a file
Savefig = 1     # Save figures as png
plotlist = [1,2,3,5,6]
...
# interpolation
timestep = 0.1
delays_uniform = np.arange(delays[peak],max(delays),timestep)
...
# FFT delta
sos = signal.butter(4, 2e-3, 'hp', fs=1/timestep, output='sos')
filtered_delta = signal.sosfilt(sos, delta_sa_bgremoved)
#filtered_delta = delta_sa_bgremoved
outputlen = 2**17
delta_fft = np.fft.fftshift(np.fft.fft(filtered_delta, n=outputlen))
delta_freq = np.fft.fftshift(np.fft.fftfreq(outputlen, d=0.1) * 1e3)  # in Ghz, shifted starting with negative
